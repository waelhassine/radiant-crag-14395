...
const SocieteContent = ({ getUsersJuge, user: { users, loading } }) => {
  const classes = useStyles();
  const [userRole] = React.useState('societe');
  const [age, setAge] = React.useState('');
  useEffect(() => {
    getUsersJuge(1, 'societe');
    console.log(users);
  }, [getUsersJuge]);
  const [page, setPage] = React.useState(1);
  const handleChangePaggination = (event, value) => {
    setPage(value);
    getUsersJuge(value, 'societe');
  };
  ...
  return loading ? (
    ...
    <div className={classes.root}>
      ...
      <AlertSnackbar />
      {open === true ? (
        <AddFormJuge open={open} handleClose={handleClose} userRole={userRole} />
      ) : null}
      {openTwo === true ? (
        <ConfirmBlock
          open={openTwo}
...
export default connect(mapStateToProps, { getUsersJuge })(SocieteContent);
